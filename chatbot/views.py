...
def get_dynamic_response_type_and_reply(message):
    """Ask GPT whether the message is casual and reply warmly, or return 'TAROT' to continue normal tarot flow."""
    prompt = f"""
คุณคือผู้ช่วยในแชทบอทไพ่ทาโรต์ที่ตอบโต้ได้เหมือนมนุษย์

ข้อความจากผู้ใช้: "{message}"

1. ถ้าข้อความนี้เป็นแค่การพูดคุยทั่วไป เช่น คำชม คำขอบคุณ การบอกลา หรือข้อความสุภาพทั่วไป (ที่ไม่ต้องการคำทำนายไพ่)
→ ให้ตอบกลับอย่างสุภาพและอบอุ่นในฐานะผู้ช่วยหมอดู โดยไม่ต้องพูดถึงไพ่

2. ถ้าเป็นคำถามจริงจังที่ต้องการคำทำนายจากไพ่ → ตอบแค่ "TAROT"
"""
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "คุณคือผู้ช่วยแชทบอทไพ่ทาโรต์"},
        ...
        return f"เกิดข้อผิดพลาดในการวิเคราะห์ข้อความ: {e}"
    ...
    if request.method == 'POST':
        ...
        if len(selected) == NUM_CARDS:
            ...
            if user_message:
                ...
                # 🔍 Detect casual message dynamically using GPT
                casual_reply = get_dynamic_response_type_and_reply(user_message)
                if casual_reply != "TAROT":
                    request.session['chat_history'].append({'role': 'bot', 'text': casual_reply})
                    request.session.modified = True
                    return redirect('chatbot:index')  
